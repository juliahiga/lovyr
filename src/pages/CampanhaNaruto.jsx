  ...
  return (
    <div className="personagem-card">
      ...
      <div className="personagem-card-info">
        ...
        <div className="personagem-card-nivel">{p.nivel_shinobi}</div>
        <div className="personagem-card-data">Registrado em {formatarData(p.entrou_em)}</div>
        <div className="personagem-card-sistema">Naruto: Shinobi no Sho</div>
      </div>
      <div className="personagem-card-acoes">
        <button className="personagem-acessar-btn" onClick={() => navigate(`/naruto/ficha/${p.ficha_id || p.id}`)}>
          Acessar Ficha
        </button>
      </div>
    </div>
...
const ModalConvidar = ({ campanhaId, onFechar }) => {
  const link = `${window.location.origin}/entrar-campanha-naruto/${campanhaId}`;
...
const ModalAdicionarPersonagem = ({ campanhaId, onSucesso, onFechar }) => {
  ...
  useEffect(() => {
    fetch(`${API}/api/naruto/fichas`, { credentials: "include" })
      .then((r) => r.json())
  ...
  const handleEntrar = async () => {
    ...
    setErro(null);
    try {
      const res = await fetch(`${API}/api/naruto/campanhas/${campanhaId}/entrar`, {
        method: "POST",
        credentials: "include",
      ...
      <div className="ct-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="ct-modal-body">
          {carregando ? (
            ...
            <p className="ct-modal-desc">Você não tem nenhuma ficha de Naruto criada.</p>
          ) : (
            <>
                ...
                {fichas.map((f) => (
                  ...
                  >
                    <div className="ct-ficha-avatar">
                      {f.imagem
                        ? <img src={f.imagem} alt={f.nome_personagem} />
                        : <span>{f.nome_personagem?.[0]?.toUpperCase()}</span>}
                    </div>
                    <div className="ct-ficha-info">
                      <div className="ct-ficha-nome">{f.nome_personagem}</div>
                      <div className="ct-ficha-classe">{f.nivel_shinobi}</div>
                    </div>
                  </div>
                ))}
              </div>
...
const CampanhaNaruto = () => {
  ...
  const buscarCampanha = useCallback(() => {
    setCarregando(true);
    fetch(`${API}/api/naruto/campanhas/${id}`, { credentials: "include" })
      .then((r) => r.json())
  ...
  const handleSalvarFoto = async (novaImagem) => {
    ...
    await fetch(`${API}/api/naruto/campanhas/${id}`, {
      method: "PUT",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
  ...
  const handleSairCampanha = async () => {
    await fetch(`${API}/api/naruto/campanhas/${id}/sair`, {
      method: "DELETE",
      credentials: "include",
    });
    navigate("/campanhas");
      ...
      <div className="ct-acoes-bar">
        {visitante ? (
          ...
          <>
            ...
            <button className="ct-acao-btn" onClick={() => navigate(`/escudo-mestre-naruto/${id}`)}>
